...
type Timing = {
  ...
  stalled?: { name: string; idleDays: number }[]
}

const SYSTEM = [
  'あなたは相続手続き案件の進捗を、管理担当が受注担当へ週次報告するために要約するアシスタント。',
  '入力は工程（相続人調査・財産調査・遺産分割 等）ごとにまとまった業務の一覧で、各業務に状態（完了/進行中/未着手）とメモが付く。',
  'さらに「時間・進み具合」として、着手からの経過日数・最終更新からの日数・期限超過タスク・長期停滞タスクが与えられることがある。',
  '出力は必ず次の形式のJSONのみ（前置き・コードフェンス・見出し・絵文字なし）:',
  '{"overall":"全体を2〜3文","byKotei":{"<工程名>":"その工程を1文",...}}',
  'overall: ①案件全体の現在地・並行して動いている業務・次の山、②進み具合の評価（着手から約○日経過／期限超過タスクの有無と件数／長期停滞している作業があればその旨）を、合わせて2〜3文で。進行中を優先し、未着手はまとめて軽く。',
  '進み具合は与えられた数値の範囲で述べる。期限超過や長期停滞があれば具体名と日数に触れ、順調なら「大きな遅れはない」と述べる。憶測で原因や日付を足さない。',
  'byKotei: 各工程について「今どこまで進み、次に何をするか」を1文で。完了済みの工程は完了した旨を簡潔に。',
  'byKotei のキーは入力に出てきた工程名を厳密にそのまま使う。入力に無い工程は作らない。',
  '事実（状態・メモ・時間情報）に無いことは書かない。憶測で日付や固有名詞を足さない。',
].join('\n')

// 「時間・進み具合」ブロックを組み立てる（無ければ空文字）
function buildTimingBlock(t?: Timing): string {
  if (!t) return ''
  const lines = [
    t.elapsedDays != null ? `- 着手からの経過：約${t.elapsedDays}日${t.startDate ? `（起点 ${String(t.startDate).slice(0, 10)}）` : ''}` : null,
    t.daysSinceLastActivity != null ? `- 直近の動き：最終更新から${t.daysSinceLastActivity}日` : null,
    t.overdue && t.overdue.length
      ? `- 期限超過タスク（${t.overdue.length}件）：${t.overdue.map(o => `${o.name}（${o.days}日超過）`).join('、')}`
      : '- 期限超過タスク：なし',
    t.stalled && t.stalled.length
      ? `- 長期停滞（着手後に更新が止まっている作業）：${t.stalled.map(s => `${s.name}（${s.idleDays}日動きなし）`).join('、')}`
      : null,
  ].filter(Boolean)
  return lines.length ? `\n\n【時間・進み具合】\n${lines.join('\n')}` : ''
}
