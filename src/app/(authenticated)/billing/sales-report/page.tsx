...
export default async function SalesReportPage() {
  ...
  const [invoicesResult, expensesResult, rewardsResult, teamsResult] = await Promise.all([
    supabase
      .from('invoices')
      .select('id, case_id, invoice_type, firm_type, fee_amount, expenses_amount, amount, deduct_expense_nontax, deduct_expense_tax, posted_date, issued_date, notes, status, payments(amount, payment_date, is_refund), cases(case_number, deceased_name, bank, billing_pattern, clients(name), case_members(role, members(name, team_id)))')
