...
export default function MeetingForm({ selectedCase, currentMemberId, standalone = false, onBack, onDirtyChange, onSaved, lpLinked }: Props) {
  ...
  //   税抜を入力させて×1.1する形をやめ、伝えた額をそのまま入れてもらう。
  //   消費税の内訳は出さない（面談で要るのは伝えた税込額だけ。内訳は請求書側で計算する）。
          ...
          <Card label="提案金額">
                ...
                <div key={field} className={i > 0 ? 'mt-4 pt-4 border-t border-gray-100' : ''}>
                  ...
                  {propMode(val) === '金額を入力' && (
                    <div className="mt-3 max-w-[320px]">
                      <div className="text-[11px] text-gray-400 mb-1">税込（円）</div>
            ...
            {propTotalIncl > 0 && (
              <div className="mt-4 pt-3 border-t border-gray-200 flex items-center justify-between text-[13px]">
                ...
                <span className="tabular-nums text-gray-700">税込 <b className="text-brand-700">{propTotalIncl.toLocaleString('en-US')}</b> 円</span>
              </div>
            )}
          </Card>
