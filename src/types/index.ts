...
export type CaseStatus = '面談設定済' | '検討中' | '検討中（契約書待ち）' | '受注' | '戻り受注' | '対応中' | '完了' | '失注' | '紹介のみ'
...
export type CaseRow = {
  ...
  intake_draft?: boolean                     // migration 194: 面談シート入力途中の下書き案件（一覧/KPIから除外。②保存で false に昇格）
  ...
  intake_roles: { gyomu: string; sagyou: string; owner: string; note: string; status?: string; due?: string | null; kind?: 'task' | 'doc'; rid?: string; custom?: boolean; role_kind?: 'assistant' | 'manager' | 'sales' | 'touki'; priority?: string | null; outing?: boolean }[] | null
  ...
  // 即受注フラグ（面談登録で受注にした＝その場受注のとき true。互換維持。実体は order_win_type）
  instant_order: boolean | null
  // 受注の獲得区分（status='受注' のとき）。'即受注' | '面談なし受注' | null(=通常受注)。migration 159
  order_win_type: string | null
  ...
  // 依頼者情報タブ
  client_trait: string | null   // 依頼者の特徴（CLIENT_TRAIT_OPTIONS の複数選択をカンマ連結。以前の3値の型は実データと違っていた）
  client_trait_detail: string | null
  has_complaint: boolean
  complaint_detail: string | null
  // 作業着手準備まわり（migration 215）
  filing_status?: string | null                 // ファイル化：未 / 済
  order_sheet_finalized_at?: string | null       // OS最終化ハンコ（管理担当チェック済）
  order_sheet_finalized_by?: string | null
  order_sheet_finalized_name?: string | null
  work_prep_advanced_at?: string | null          // 作業着手準備へ進めた
  work_prep_advanced_by?: string | null
  work_prep_advanced_name?: string | null
