...
export const CASE_STATUSES = [
  ...
  { key: '対応中', label: '案件進行中', color: '#7C3AED' },
...
export const ALLOWED_STATUS_TRANSITIONS: Record<string, string[]> = {
  ...
  '失注': ['紹介のみ', '受注', '戻り受注'],
  '紹介のみ': ['受注', '戻り受注', '失注'],
  // 業務完了申請中/納品完了 は原則UIの手動遷移ではなくフロー起因で変わる（統一報告モーダル経由）。
  // ここでは復帰用の一部遷移だけ許可し、通常ドロップダウンには出さない。
  '対応中': ['業務完了申請中'],           // 業務完了申請 → 業務完了申請中
  '業務完了申請中': ['対応中', '完了'],   // 受注承認→完了 / 差戻し→対応中
