...
export const CASE_STATUSES = [
  ...
  // 受注後、作業進行中に入るまでの準備段階（管理担当割振り・OS最終化・契約書類受領・前受金入金・ファイル化）
...
export const CONSIDERATION_PERIODS = ['1週間', '2週間', '1ヶ月', '見込み不明', '四十九日以降'] as const

// 見込み度合い（検討中の受注確度）。相談結果登録・PC版で入力し cases.prospect_level に保存。
export const PROSPECT_LEVELS = ['高', '中', '低', '不明'] as const
...
export const ALLOWED_STATUS_TRANSITIONS: Record<string, string[]> = {
  ...
  '受注': ['作業着手準備', '失注'],
  '戻り受注': ['作業着手準備', '失注'],
  '作業着手準備': ['対応中', '受注', '失注'],   // 事務管理ダッシュボードの着手OKで対応中へ（ドロップダウンでも可）
  '失注': ['紹介のみ', '受注', '戻り受注'],
  '紹介のみ': ['受注', '戻り受注', '失注'],
...
export const getSelectableCaseStatuses = (
  ...
  const canManage = (orderSheetCompleted && managerAssigned && contractProcDone) || isManagementNow
...
//   印鑑登録証明書             … 原本をお預かりして最後にお返しする（区分＝お客様預かり書類）
// 「お客様預かり書類」は納品タブの対象になるので、返却漏れがそのまま拾える。
// 逆に言うと、返さないものをこの区分にすると原本受領証に載ってしまう。
...
//   印鑑登録証明書は区分が「お客様預かり書類」になったが、契約時に集めるものなので
...
// === 不動産: 物件区分 ===
// 物件種別。エクセル準拠で 土地／建物／マンション の3択に集約。
// 既存データの旧種別（戸建/農地/山林/駐車場/収益物件/その他）は各選択欄で現在値をフォールバック表示して保持する。
...
// === 難易度（3段階）と難しい理由（複数選択） ===
export const DIFFICULTY_LEVELS = ['普通', '難', '激難'] as const
