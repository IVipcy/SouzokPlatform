...
export const INVOICE_FIELDS = {
  ...
  subtotalCell: 'B23',                // 「小計」の見出し（B23:T23の結合。中央に寄せる）
  // 発行年月日。テンプレは「令和 [ ] 年 [ ] 月 [ ] 日」の形で、数字だけ空いている。
  //   R8:S8='令和' / T8=年 / U8='年' / V8=月 / W8='月' / X8=日 / Y8='日'
  issueDate: { era: 'R8', year: 'T8', month: 'V8', day: 'X8' },
  // 振込先の口座番号。テンプレに法人ごとの番号が「数値」で入っている（M38:O38の結合）。
  // 数値は幅が足りないと Excel が #### や指数表記に切り替えるため、桁が読めなくなっていた。
  // 文字列にすると、この切り替えが起きない（入りきらない場合は端が隠れるだけ）。値は変えない。
  bankAccountCell: 'M38',
}
