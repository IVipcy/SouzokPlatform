...
// 業務区分バッジ（固定幅・薄グレー）。
export function GyomuBadge({ phase, width = 64 }: { phase: string | null | undefined; width?: number }) {
  const g = (phase ?? '').replace(/^Phase\d+[:：]\s*/, '').trim()
  if (!g) return <span className="text-gray-300 text-[12px]">—</span>
  return (
    <span
      className="inline-block text-center text-[11px] font-semibold py-0.5 rounded-md flex-shrink-0 truncate align-middle text-gray-600 bg-gray-100/80 border border-gray-200"
      style={{ width }}
      title={g}
    >
      {g}
