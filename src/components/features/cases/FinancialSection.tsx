  ...
  return (
    <div className="space-y-3">
      ...
      {active && arrivalId && (() => {
        ...
        return <FinancialArrivalModal isOpen onClose={() => setArrivalId(null)} request={r} items={allItems.filter(it => it.request_id === r.id)} accounts={accountsOf(active)} institutionName={active.name} deceasedName={caseData.deceased_name} onSaved={() => onRefresh?.()} />
      })()}
    </div>
  )
}
...
function TopTable({ layout, institutions, evalOf, accountsOf, holdings, requests, items, tasks, jasdecRows, onOpen }: {
  ...
  // 縦に結合したセルは2行目以降で td+td の縦線が付かないので、結合セル自身に左右の線を持たせる
