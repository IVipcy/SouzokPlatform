...
export default function ClientInfoTab({ caseData, clientCommunications, patchCase, patchClient, onRefresh, orderSheetMode = false, caseClients = [] }: Props) {
  ...
  // 顧客郵送先に選んだ依頼者（case_clients.id）。選択時はその人の住所を郵送先に自動表示。
  const mailingRecipient = caseClients.find(c => c.id === caseData.mailing_destination)
  ...
  return (
    <div className="space-y-3.5">
      ...
      {/* 2. 郵送・書類設定（顧客郵送先＝誰に送るかを依頼者から選択→その人の住所を自動表示） */}
      <Section title="郵送・書類設定" collapsible defaultOpen={orderSheetMode}>
        <FieldGrid>
          <InlineSelect
            label="顧客郵送先（誰に送るか）"
            value={caseData.mailing_destination}
            options={[...caseClients.map(c => c.id), 'その他']}
            optionLabel={v => v === 'その他' ? 'その他' : (caseClients.find(c => c.id === v)?.name || '(不明な依頼者)')}
            onSave={v => saveCaseField('mailing_destination', v)}
          />
          {mailingRecipient ? (
            <Field label="郵送先住所（自動）" value={[mailingRecipient.postal_code, mailingRecipient.address].filter(Boolean).join('　') || '住所未登録（依頼者一覧で入力）'} />
          ) : caseData.mailing_destination === 'その他' ? (
            <InlineEdit label="郵送先住所（その他）" value={caseData.mailing_address_other} onSave={v => saveCaseField('mailing_address_other', v)} fullWidth />
          ) : null}
        </FieldGrid>
        <p className="text-[11px] text-gray-400 mt-2">郵送先に依頼者を選ぶと、その依頼者の住所（依頼者一覧で登録）が自動で郵送先住所になります。同行者宛にも選べます。</p>
      </Section>
