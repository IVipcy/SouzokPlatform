...
export default function OrderSheet({
  ...
  const allOsSections: { title: string; gate?: TabKey; node: ReactNode }[] = [
    ...
    ...PROCEDURE_TABS.map(p => ({
      title: p.title,
      gate: p.tab,
      node: <PracticeProcedureTab caseData={caseData} patchCase={patchCase} gyomu={p.gyomu} title={p.title} description={p.description} court={p.court} trust={p.trust} mediation={p.mediation} heirs={heirs} sagyoDocuments={sagyoDocuments} receipts={receipts} onRefresh={onRefresh} embedded />,
    })),
