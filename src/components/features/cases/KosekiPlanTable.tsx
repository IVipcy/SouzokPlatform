...
export default function KosekiPlanTable({ caseId, caseData, heirs }: Props) {
  ...
  const save = async (name: string, field: 'range_text' | 'address_doc' | 'note' | 'acquisition_authority' | 'priority', value: string) => {
    ...
    const target: Partial<Record<'acquisition_authority' | 'range_text' | 'notes', string>> | null =
      field === 'acquisition_authority' ? { acquisition_authority: v ?? undefined }
      : field === 'range_text' ? { range_text: v ?? undefined }
      : field === 'note' ? { notes: v ?? undefined } : null
    if (target && v) {
      const col = Object.keys(target)[0] as 'acquisition_authority' | 'range_text' | 'notes'
      const { data: reqs } = await supabase.from('koseki_requests').select(`id, target_person, request_date, ${col}`).eq('case_id', caseId)
      const ids = ((reqs ?? []) as unknown as Array<Record<string, string | null>>)
        .filter(r => !r[col] && !r.request_date && normalizePersonName(r.target_person ?? '') === normalizePersonName(key)).map(r => r.id as string)
      if (ids.length > 0) await supabase.from('koseki_requests').update({ [col]: v }).in('id', ids)
    }
  }
