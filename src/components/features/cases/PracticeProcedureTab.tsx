...
import type { CaseRow, HeirRow, SagyoDocumentRow } from '@/types'
...
export default function PracticeProcedureTab({ caseData, patchCase, gyomu, title, description, court, trust, mediation, heirs = [], embedded, sagyoDocuments = [], receipts = [], onRefresh }: Props) {
  ...
  const addTaskRow = () => save([...roles, { gyomu, sagyou: '', owner: '自社', note: '', status: '未着手', due: null, kind: 'task' }])

  // 役割分担から作業が無い場合、マスタから標準作業（kind付き）を読み込む
  const seedFromMaster = () => {
    const seeded: RoleRow[] = tasksFor(caseData.service_category ?? '', gyomu)
      .map(t => ({ gyomu, sagyou: t.task, owner: '自社', note: '', status: '未着手', due: null, kind: kindOf(t) }))
    save([...roles, ...(seeded.length ? seeded : [{ gyomu, sagyou: '', owner: '自社', note: '', status: '未着手', due: null, kind: 'task' as const }])])
  }
              ...
              <table className="w-full text-[13px] border-collapse" style={{ minWidth: 720 }}>
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200 text-[12px] text-gray-500">
                    <th className="px-2.5 py-2 text-left font-semibold w-80">作業</th>
