...
export default function ProgressKpis({ scopeLabel, metrics }: Props) {
  ...
  const KPIS = [
    ...
    { label: '青色件数', value: String(metrics.blueCount), unit: '件', desc: 'アラートが1つも出ていない順調な案件', tone: 'blue' },
    { label: '黄色件数', value: String(metrics.yellowCount), unit: '件', desc: '要確認（黄）のアラートが出ている案件。赤・紫のアラートは無い状態', tone: 'yellow' },
    { label: '赤色件数', value: String(metrics.redCount), unit: '件', desc: '要注意（赤）のアラートが1つ以上出ている案件。何が赤いかは案件詳細のバッジに出ます', tone: 'red' },
    { label: '紫色件数', value: String(metrics.purpleCount), unit: '件', desc: 'クレームが発生している案件。赤よりさらに優先して対応する', tone: 'purple' },
  ...
  return (
    <section className="mb-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-[11.5px] text-gray-400">案件の色＝出ているアラートで一番重い色（紫＞赤＞黄＞青）</span>
      </div>
