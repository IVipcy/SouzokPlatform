  ...
  return (
    <nav className="mb-5 bg-white border border-gray-200 rounded-lg p-3">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[10px] font-semibold text-gray-500 mr-1">メンバー切替：</span>

        {/* チーム全体へのリンク */}
        <Link
          href={`/dashboard/team/${teamId}/progress`}
          className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1.5 rounded border transition ${
            !currentMemberId
        ...
        {sorted.map(m => {
          ...
          return (
            <Link
              ...
              className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-1.5 rounded border transition ${
                active
                  ? 'bg-blue-600 text-white border-blue-600'
                  : 'bg-white text-gray-700 border-gray-300 hover:border-blue-400 hover:bg-blue-50/40'
              }`}
            >
              ...
              <span>{m.name}</span>
              <span
                className={`text-[8px] font-mono px-1 py-0 rounded border ${active ? 'bg-white/20 text-white border-white/30' : role.cls}`}
              >
                {role.label}
              </span>
            </Link>
