  ...
  return (
    <section className="mb-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-[13px] text-gray-500 px-2 py-0.5 bg-gray-100 rounded">当月の受注担当の数値</span>
      </div>
        ...
        {KPI_DEFS.map(def => (
          <div key={def.key} className="bg-white border border-gray-300 rounded-xl overflow-hidden">
            <div className="px-2 py-2 border-b border-gray-200 bg-gray-50 text-center">
              <div className="text-[13px] font-semibold text-gray-700">{def.label}</div>
            </div>
            <div className="px-2 py-3 text-center">
              <div className="flex items-baseline justify-center gap-0.5">
                ...
                <span className="text-[14px] font-bold text-gray-500">{def.unit}</span>
              </div>
            </div>
            <div className="px-2 py-2 bg-gray-50 border-t border-gray-100 min-h-[42px]">
              <p className="text-[13px] leading-snug text-gray-500">{def.desc}</p>
            </div>
          </div>
        ))}
      </div>
