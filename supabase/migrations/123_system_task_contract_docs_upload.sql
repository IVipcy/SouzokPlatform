-- ============================================================
-- 123_system_task_contract_docs_upload.sql
-- 受託（受注）時の初期システムタスクに「契約時受領書類のアップロード」(sys_contract_docs_upload) を追加。
--   契約時にお客様から受領した書類（戸籍・通帳コピー・固定資産税通知書 等）をスキャンして添付する作業。
--   category='初期対応' なので、受託→対応中のゲート（初期対応タスク全完了）に含まれ、
--   アップロードしないと対応中に進めない＝添付し忘れを防ぐ。
--   生成タイミング: 受注（＝検討中（契約書待ち）→受託 の遷移）。重複ガードで二重生成しない。
-- 関数 generate_system_tasks_on_status_change() を差し替え（095 に上記タスクを追加するのみ・非破壊）。
-- ※ 初期タスク確認ポップアップ側は INITIAL_TASK_KEYS['受注'] に 'sys_contract_docs_upload' を追加する。
-- ============================================================
...
BEGIN
  ...
  IF NEW.status = '受注' THEN
    ...
    -- 初期対応タスクなので、対応中へ進むゲート（全完了）に含まれる＝アップロードし忘れを防ぐ。
    PERFORM create_system_task(
